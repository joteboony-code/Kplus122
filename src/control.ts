...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(circle at 15% 0,#163e29 0,transparent 36%),#07100c;padding:22px}
    .shell{width:min(100%,1440px);margin:24px auto}.top{display:flex;justify-content:space-between;align-items:center;margin-bottom:24px}.brand{display:flex;gap:13px;align-items:center}.mark{display:grid;place-items:center;width:46px;height:46px;border-radius:14px;background:#25d366;color:#07110c;font-size:21px;font-weight:900}.brand strong{display:block;font-size:19px}.brand span{color:#789486;font-size:13px}
    ...
    .dashboard-grid{display:grid;grid-template-columns:minmax(420px,.9fr) minmax(0,1.35fr);gap:24px;align-items:start}.info-column,.logs-column{min-width:0}.logs-column{padding-left:24px;border-left:1px solid #294537}
    ...
    @media(max-width:1050px){.dashboard-grid{grid-template-columns:1fr}.logs-column{padding-left:0;border-left:0;border-top:1px solid #294537;padding-top:18px}.pipeline{grid-template-columns:repeat(2,1fr)}.step:nth-child(2)::after{display:none}.log-main{grid-template-columns:84px 68px 76px minmax(180px,1fr) 92px;column-gap:8px;row-gap:4px}.log-meta{grid-column:1/-1;display:flex;justify-content:flex-end;gap:8px}}
...
<body><main class="shell"><header class="top"><div class="brand"><div class="mark">K+</div><div><strong>KPLUS Control</strong><span>LINE receipt inspection</span></div></div><form class="logout" method="post" action="/control/logout"><button type="submit">ออกจากระบบ</button></form></header>
  ...
  <div class="notice">OCR.space นับตามวันที่ประเทศไทย ส่วน Google Vision และ Queue เป็นค่าประมาณจากตัวนับของ Worker นี้ ตัวนับ Queue เริ่มเก็บตั้งแต่เวอร์ชันที่เปิดใช้การแสดงผล จึงไม่รวมยอดก่อนหน้านี้หรือระบบอื่นในบัญชี Cloudflare</div></div><div class="logs-column">
  ...
  <div class="logs" id="inspection-logs">${recentLogs}</div></div></div></section></main><script>
  </script></body></html>`;
}
