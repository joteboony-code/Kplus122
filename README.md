...
Cloudflare Worker สำหรับรับรูปจาก LINE Messaging API และตรวจสลิปเป็นรอบต่อผู้ส่ง โดยใช้ `message.imageSet.id` ของชุดรูป 10+ ภาพจาก LINE เป็นรหัสรอบโดยตรง หนึ่งรอบต้องพบสลิปคนละ 2 ใบ:
...
2. สลิป `KBANK` ต้องมี `SETTLEMENT` และมียอดเงินเท่าใดก็ได้

รูปเดียวจะไม่ถูกนับเป็นทั้งสองประเภท เพื่อไม่ให้หัวกระดาษ `K+ KBank` ผ่านแทนสลิป KBANK อีกใบ
...
ระบบประมวลผลทีละหนึ่งรูป ป้องกัน Webhook/รูปเดิมซ้ำ 7 วัน แยกสถานะตามรหัสชุดรูปของ LINE และหยุดตรวจรูปถัดไปของผู้ส่งเดิมในกลุ่มเดิม 60 วินาทีหลังพบครบสองใบ หาก LINE ไม่ส่งรหัสชุดรูป ระบบจึงใช้ผู้ส่งและช่วงเวลา 5 นาทีเป็น fallback
