...
เมื่อพบสลิป KPLUS ที่ถูกต้อง ระบบตอบผลผ่านกลับ LINE ด้วย Reply API แล้วบันทึกผลและปิดรอบตรวจ ส่วนผลตรวจไม่ผ่านจะแจ้งเตือนตามปกติ
...
เมนู `ตรวจงาน Service` และ `งาน Service ทั้งหมด` แสดงรายการงานโดยไม่แท็กช่าง การแท็กจะเกิดขึ้นเฉพาะตอนตอบผลตรวจสลิปผ่านหรือไม่ผ่าน: ระบบตรวจงานใหม่ที่ตรงพื้นที่ของผู้ส่งรูป แนบข้อความแท็กและการ์ดงานเข้าไปใน Reply API ครั้งเดียวกัน และจำงานที่เคยแจ้งแยกตาม LINE User ID เพื่อไม่แจ้งงานเดิมซ้ำ หากอ่านงาน Service ไม่สำเร็จ ผลตรวจสลิปยังตอบตามปกติ

ทุกครั้งที่มีการส่งรูป ระบบจะแนบ Flex `เปิด Stock` พร้อม Tid 8 ตัวล่าสุดของผู้ส่งเพียงครั้งเดียวต่อรอบด้วย Reply API โดยแนบกับผลตรวจผ่าน/ไม่ผ่าน หรือส่ง Flex อย่างเดียวเมื่อไม่มีผลตรวจ หากไม่พบ Tid จะแสดง `Tid: ไม่ระบุ` ส่วนคำสั่ง `Check Service` และ `Service ทั้งหมด` ไม่เพิ่ม Flex Stock แยก เพราะมี Quick Reply อยู่แล้ว
...
4. เมื่อผ่าน ระบบต้องตอบ `ตรวจสอบผ่าน` และบันทึกผลตามปกติ
5. ส่งสลิป KPLUS ที่ยอดไม่ถูกต้อง ระบบต้องแจ้ง `ตรวจสอบไม่ผ่าน` พร้อมสาเหตุ

## การตอบล่าช้า

ระบบประมวลผลผ่าน Queue แล้วพยายามใช้ Reply token สำหรับผลตรวจทั้งผ่านและไม่ผ่านก่อน เพื่อประหยัดโควตาข้อความ หาก Queue ล่าช้าจน Reply token ใช้ไม่ได้ ระบบจะบันทึก error ไว้ แต่จะไม่ Push ซ้ำโดยค่าเริ่มต้น

หากต้องการให้ Push ผลไม่ผ่านกลับกลุ่มเมื่อ Reply ไม่สำเร็จ ให้เปลี่ยน `ENABLE_PUSH_FALLBACK` เป็น `true` ใน `wrangler.jsonc` แล้ว Deploy ใหม่ โปรดตรวจโควตาข้อความ LINE Official Account ก่อนเปิดใช้
